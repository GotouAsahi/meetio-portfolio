<template>
  <div class="carousel">
    <div @click="carouselPrev" class="carousel_prev"
      :class="{ no_carousel_prev: (likeportfolios.length <= 4) || (carouselIndex === 0) }">
    ...
    <div class="carousel_list_outer">
      ...
      <div class="carousel_list" ref="margin_search" :style="{ transform: 'translate(-' + carouselIndex + '00%, 0)' }">
        <div>[[this.portfolios||""]]</div>
        <div v-for="portfolio in likeportfolios" :key="portfolio.id" class="carousel_item" :ref="portfolio.id"
    ...
    <div @click="carouselNext" class="carousel_next"
      :class="{ no_carousel_next: (likeportfolios.length <= 5) || (carouselIndex === carouselIndexMax) }">
...
export default {
  ...
  props: ["likeportfolios", "currentuser"],
  data() {
    return {
      user: [],
      like_list: {
  ...
  computed: {
    carouselIndexMax() {
      this.InitialValue();
      if (this.likeportfolios.length % 5 == 0) {
        return parseInt(this.likeportfolios.length / 5 - 1);
      } else {
        return parseInt(this.likeportfolios.length / 5);
      }
    }
  },
  mounted() {
