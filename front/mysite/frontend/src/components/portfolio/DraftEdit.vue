<template>
  <div class="article-post mx-auto my-10 w-10/12">
    <form class="border-1 border-gray-900" v-on:submit.prevent="UpdatePortfolio">
      <div class="mt-10 mx-auto w-full first-line:max-w-sm bg-white border-t border-x border-gray-200 rounded-t-3xl">
        <div class="flex items-center justify-between mx-4 pt-4 pb-3 px-8 border-b-2">
          <h3 class="text-3xl font-bold text-teal-500 mt-3 tracking-widest">投稿</h3>
        </div>
        <div class="py-8 px-12">
          ...
          <div class="mt-10 mb-1">
            ...
            <div class="flex flex-row items-center text-base sm:flex-row sm:space-y-0 sm:space-x-4">
              ...
              <input type="button" v-on:click="addTag" value="追加"
      ...
      <!--マークダウン-->
      <div class="mt-10 mx-auto w-full first-line:max-w-sm bg-white rounded-t-xl shadow">
        <div class="flex items-center justify-center text-teal-500 text-xl font-semibold mx-6 py-3 px-2 gap-20">
          <label>本文</label>
          <div class="block text-lg text-left text-red-500">＊必須</div>
        </div>
      </div>
      ...
      <div class="mt-10 flex justify-center text-center gap-20">
        <input type="button" v-on:click="UpdateDraft" value="下書きを更新"
        ...
        <input type="button" v-on:click="DeleteDraft('DraftList')" value="下書きを削除"
          class="text-white bg-gradient-to-r from-teal-400 via-teal-500 to-teal-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-teal-400 shadow-lg shadow-teal-500/70 font-medium rounded-full text-lg w-1/5 px-5 py-2.5 text-center mr-2 mb-2" />
      </div>
    </form>
  </div>
...
export default {
  ...
  methods: {
    getDraft() {
      axios
        .get(
          `/api/v1/portfolio/mydraft/${this.$route.params.id}`,
          {
    ...
    AddPortfolio() {
      ...
      if (this.portfolio.title_image == null) {
        this.portfolio.title_image = "";
      }
      formData.append("title_image", this.portfolio.title_image);
      if (this.portfolio.movie == null) {
    ...
    UpdateDraft() {
      ...
      console.log("usedImages:", this.sentenceImage);
      this.sentenceImage.forEach(image => {
        if (!this.portfolio.sentence.includes(image.url)) {
          this.deleteImageFromServer(image);
        }
      ...
      axios
        .put(
          `/api/v1/portfolio/draft/${this.portfolio.id}/`,
          formData,
          {
            headers: {
              "Content-Type": "multipart/form-data"
    ...
    DeleteDraft(flag) {
      axios
        .delete(`/api/v1/portfolio/draft/${this.$route.params.id}/`)
        .then(() => {
          if (this.selectedTitleImage) {
          ...
          //下書きを投稿するときはTopListに移動するので削除ボタンを押したときだけDraftListに移動する
          if (flag === 'DraftList') {
